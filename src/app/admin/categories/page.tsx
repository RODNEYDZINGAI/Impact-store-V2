    ...
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Manage storefront navigation while preserving legacy product categories.</p>
        </div>
        <Button onClick={save} disabled={status === "saving"} className="bg-[#1f4f8f] text-white hover:bg-[#1f4f8f]/90">
          {status === "saving" ? "Saving..." : "Save taxonomy"}
        </Button>
      </div>

      {status === "saved" && <p className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">Category taxonomy saved.</p>}
      {error && <p className="mt-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</p>}

      <div className="mt-6 space-y-5">
        {categories.map((category, categoryIndex) => (
          <section key={`${category.slug}-${categoryIndex}`} className="rounded-2xl border border-slate-200 bg-white p-5">
            <div className="flex items-start justify-between gap-4">
              <div className="grid flex-1 grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                  <label className="block text-sm font-medium text-slate-500">Category name</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-500">Default legacy category</label>
              ...
              <button type="button" onClick={() => removeCategory(categoryIndex)} className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700 hover:bg-red-100">Remove</button>
            </div>

            <div className="mt-5">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-semibold uppercase tracking-wider text-slate-500">Subcategories</h2>
              ...
              <div className="mt-3 space-y-3">
                {category.subcategories.map((subcategory, subcategoryIndex) => (
                  <div key={`${subcategory.slug}-${subcategoryIndex}`} className="grid grid-cols-1 gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 md:grid-cols-[1fr_1fr_1fr_auto]">
                    ...
                    <button type="button" onClick={() => removeSubcategory(categoryIndex, subcategoryIndex)} className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700 hover:bg-red-100">Remove</button>
                  </div>
                ))}
              </div>
            </div>
